...
macro_rules! impl_from_request {
    (
        ...
        paste! {
            ...
            impl<S, $($ty,)* $last, $([<$ty Via>],)* [<$last Via>]> FromRequestParts<S, ($([<$ty Via>],)* [<$last Via>],)> for ($($ty,)* $last,)
            where
                $( $ty: FromRequestParts<S, [<$ty Via>]> + Send, )*
                $last: FromRequestParts<S, [<$last Via>]> + Send,
                S: Send + Sync,
            {
            ...
            #[allow(non_snake_case, unused_mut, unused_variables)]
            impl<S, $($ty,)* $last, $([<$ty Via>],)* [<$last Via>]> FromRequest<S, ($([<$ty Via>],)* [<$last Via>],)> for ($($ty,)* $last,)
            where
                $( $ty: FromRequestParts<S, [<$ty Via>]> + Send, )*
...
mod tests {
    ...
    #[test]
    fn unit() {
        assert_from_request_parts::<_, ()>();
        assert_from_request::<_, ()>();
    }

    #[test]
    fn tuple_of_one() {
        assert_from_request_parts::<_, (Method,)>();
