...
/// are sent by HTML checkboxes and multi selects. Those values can be collected into a `Vec`.
///
/// # Example
///
/// ```rust,no_run
    ...
    B::Error: Into<BoxError>,
{
    ...
    async fn from_request(req: &mut RequestParts<B>) -> Result<Self, Self::Rejection> {
        if req.method() == Method::GET {
            ...
            if !has_content_type::has_content_type(req, &mime::APPLICATION_WWW_FORM_URLENCODED) {
...
mod has_content_type {
    include!(concat!(
        ...
        "/../axum/src/extract/has_content_type.rs"
    ));
}
